...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$correo) {
        ...
        if (mysqli_num_rows($resultado_verificar_duplicados) > 0) {
            $mensajeError = 'No se puede agregar al usuario, alguno de los datos proporcionados ya están en uso.';
            ...
            try {
                ...
                $query = "INSERT INTO Usuarios (NombreUsuario, Contra, DocumentoIdentidad, RolID, Correo, Activo, Nombre)
                      VALUES ('$nombreUsuario', '$hashedContra', '$documentoIdentidad', 3, '$correo', 1, '$nombreCompleto')";
                $resultado = mysqli_query($conexion, $query);

                if ($resultado) {
                    echo "<script>alert('Financiero agregado exitosamente.');window.location.href='gestionfinancieros.php';</script>";
                } else {
...
<body class="gradient-custom-1">
    ...
    <div class="mx-4 my-4">
        <a class="btn text-white btn-lg btn-floating" data-mdb-ripple-init style="background-color: #ac2bac;"
            role="button" onclick="window.location.href='gestionfinancieros.php'">
                    ...
                    <div class="card near-moon-gradient form-white" style="border-radius: 1rem;">
                        <div class="card-body p-5">
                            <form method="POST" action="">
                                <?php
                                if (!empty($usuariosRepetidos)) {
                                    echo '<p>Los siguientes campos ya están en uso: ' . implode(', ', $usuariosRepetidos) . '</p>';
                                }
                                ?>
                                <h3 class="text-center indigo-text font-bold py-4 fw-bold text-uppercase">
                                    <strong style="color: #fff">Agregar Financiero</strong>
                                </h3>
                                    ...
                                    <div class="d-flex flex-row-reverse justify-content-center">
                                        <div class="order-0 p-2">
                                            <button data-mdb-ripple-init type="submit" class="btn btn-light">
                                                <i class="fas fa-user-plus pe-2"></i>Agregar Financiero</button>
                                        </div>
                                        <div class="order-1 p-2">
                                            <button data-mdb-ripple-init type="button" class="btn btn-danger"
                                                onclick="window.location.href='gestionfinancieros.php'">
